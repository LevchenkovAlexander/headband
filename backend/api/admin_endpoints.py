...
@router.patch("/update_organization", response_model=StatusResponse)
...
@router.delete("/delete_organization", response_model=StatusResponse)
async def delete_organization(id: uuid.UUID,
                                session: AsyncSession = Depends(get_db_session)):
    status = await db_functions.delete_organization(delete_id=id, session=session)
    return {"status": status}

@router.post("/create_price_position", response_model=IDResponse)
...
@router.delete("/delete_price_position", response_model=StatusResponse)
...
@router.get("/cabinet", response_model=AdminResponseInfo)
...
@router.patch("/update_offer", response_model=StatusResponse)
...
@router.delete("/delete_offer", response_model=StatusResponse)
